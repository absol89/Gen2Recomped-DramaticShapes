...
local function updatePlayerTrainer(battle, mode)
  ...
  if not def or (not battle[imageField]
      and not trainerStates[battle]) then
    restoreTrainer(battle)
    return
  end
  local state = trainerStates[battle]
  ...
  if not state then
    ...
    state = { kind = "animated", def = def, mode = mode,
              ...
              originalTrueColor = trueColorField and battle[trueColorField],
              gen2 = gen2, frames = frames, frame = 1 }
    trainerStates[battle] = state
  ...
  if state.gen2 then
    if visible then
      ...
      -- Gen 1's SlideTrainerPicOffScreen is nine 8px steps, one step every
      -- two 60 Hz frames: 72 pixels over 18 frames. Gen 2 removes its trainer
      -- field immediately at send-out, so retain only our world-frame copy for
      -- the same authored exit while the engine proceeds with the send-out.
...
-- The staged Gen 2 renderer needs the retained trainer image after the native
-- showPlayerTrainer flag falls at send-out. The second result is the Gen 1
-- compatible leftward offset in pixels.

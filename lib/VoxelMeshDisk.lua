...
local STATIC_PLAYTHROUGH = "bavc_g2_static_mesh_v1"
...
-- Revision 19 closes every authored forest perimeter with explicit sides.
Disk.CACHE_REVISION = 19
...
Disk.PRECACHE_FAILURE_FILE =
  "mod-derived/BATTLE_ART_VOXEL_GEN2/precache-failures.tsv"

local LEGACY_BASE =
  "mod-derived/BATTLE_ART_VOXEL_GEN2/static-mesh-cache-g2-v1"
local LEGACY_ROOT = LEGACY_BASE .. "/unbound"
local boundVersion
...
local function selectedGameVersion(game)
...
local function bindVersion(game)
  local version = selectedGameVersion(game)
  ...
  boundVersion = segment
  LOGICAL_DIRECTORY = BASE_DIRECTORY .. "/" .. segment
  LEGACY_ROOT = LEGACY_BASE .. "/" .. segment
  Disk.DIRECTORY = LOGICAL_DIRECTORY
  return version
end

local function bindStorage(game, version)
...
function Disk.bind(game, selected)
  ...
  knownSizes = {}
  local version = bindVersion(game)
  ...
  if bindStorage(game, version) then return true end
...
function Disk.fingerprint(map, slot, masks, kind)
  ...
  local parts = {
    ...
    "atlasProfile", tostring(map._battleArtGen2AtlasProfile or "-"),
  }
...
function Disk.purge()
  ...
  -- engines. Prefer its delete primitive; older facades fall back to an empty
  -- body, which ordinary validation treats as absent.

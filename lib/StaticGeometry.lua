...
local staticWorld = nil
local configuredGame = nil
...
-- Game2 owns imported overworld registries under namespaced keys. The
-- src.core.Game compatibility facade exposes maps/tilesets aliases later,
-- but mods.loaded deliberately hands us the raw dataset before that facade is
-- involved. Prefer the Gen2 catalogs and retain Gen1-shaped fixtures/support.
local function catalogs(data)
  if type(data) ~= "table" then return nil, nil end
  local mapDefs = populated(data.gen2Maps) and data.gen2Maps or data.maps
  local tilesetDefs = populated(data.gen2Tilesets)
    and data.gen2Tilesets or data.tilesets
  return mapDefs, tilesetDefs
end
...
StaticGeometry.EXCLUSION_FILE =
  "mod-derived/BATTLE_ART_VOXEL_GEN2/static-cache-exclusions.tsv"
...
local function writeLedger()
  ...
  local text = table.concat({
    "# BATTLE ART VOXEL GEN2 static-cache exclusion ledger",
  ...
  local ok = pcall(function()
    assert(fs.createDirectory("mod-derived/BATTLE_ART_VOXEL_GEN2"))
...
local function geometryDifferences(map)
  ...
  local canonical = maps[map.id]
  local tileset = canonical and canonical.tileset
    or (def and snapshot.tilesets[def.tileset])
  if not def then
  ...
  if canonical
     and map._battleArtGen2AtlasProfile
       ~= canonical._battleArtGen2AtlasProfile then
    out[#out + 1] = { "tileset.atlas", liveDef.tileset,
                      "map-specific roof/atlas profile changed at runtime" }
  end
...
function StaticGeometry.capture(data)
  if snapshot then return false end
  local mapDefs, tilesetDefs = catalogs(data)
  if not (mapDefs and tilesetDefs) then return false end
  snapshot = {
    maps = clone(mapDefs),
    tilesets = clone(tilesetDefs),
    roofs = clone(data.gen2Roofs or data.roofs),
    palettes = clone(data.gen2Palettes or data.palettes),
  }
  maps = {}
  return true
end

-- Supply the renderer-free canonical registry with the same Gen2 atlas
-- composer used by the live overworld. This must run before map() materializes
-- any title-precache maps; reconfiguration safely discards private map objects.
function StaticGeometry.configure(game)
  if not snapshot then return false end
  if staticWorld and configuredGame == game then return true end
  local ok, Adapter = pcall(function()
    return V and V.require and V.require("Gen2WorldAdapter")
      or require("lib.Gen2WorldAdapter")
  end)
  if not (ok and Adapter and type(Adapter.staticWorld) == "function") then
    return false
  end
  local gameData = game and game.data or {}
  local roofs = populated(snapshot.roofs) and snapshot.roofs
    or gameData.gen2Roofs or gameData.roofs
  local palettes = populated(snapshot.palettes) and snapshot.palettes
    or gameData.gen2Palettes or gameData.palettes
  local data = {
    gen2Tilesets = clone(snapshot.tilesets),
    gen2Roofs = clone(roofs),
    gen2Palettes = clone(palettes),
  }
  local world = Adapter.staticWorld(game, data)
...
function StaticGeometry.map(id)
  ...
  if staticWorld then
    ...
    if ok and Adapter and Adapter.prepareMap then
      Adapter.prepareMap(staticWorld, map)
    end
  end
  maps[id] = map
  return map

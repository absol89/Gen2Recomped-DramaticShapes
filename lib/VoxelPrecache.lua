...
-- Extracted Gen2 connections carry both a numeric ROM map index in `map` and
-- the registry key in `mapId`. Only the latter indexes gen2Maps. Gen1 uses a
-- string `map`, so accept that as the fallback without ever treating the ROM
-- number as a catalog id.
...
function Precache.candidates(data, overworld)
  ...
  for dir, connection in pairs(map.def.connections or {}) do
    local other = connectionId(connection)
    if other and maps[other] then
      addCandidate(out, seen, other,
...
function Precache.masksFor(data, mapId)
  ...
  local out = {}
  for _, nb in ipairs(Controller.computeNeighbors(maps, mapId, 2) or {}) do
...
function Precache.allJobs(data)
  ...
  for id, def in pairs(maps) do
    -- Registry metadata such as `_romInfo` is table-shaped but is not a map.
    -- Requiring the fields Map.new/ChunkMesher actually consume keeps those
    -- records out of the progress total and failure report.
    local isMap = type(id) == "string" and type(def) == "table"
      and def.id == id and type(def.tileset) == "string"
      and type(def.width) == "number" and type(def.height) == "number"

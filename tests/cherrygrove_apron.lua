...
local legacyRoutes = {
  { map = { id = "ROUTE29", tileset = { blocks = blocks } },
    canonical = route29, bx = -5, by = -36 },
  { map = { id = "ROUTE30", tileset = { blocks = blocks } },
    canonical = route30, bx = 10, by = -9 },
  { map = { id = "ROUTE31", tileset = { blocks = blocks } },
    canonical = route31, bx = 20, by = 0 },
  { map = { id = "ROUTE46", tileset = { blocks = blocks } },
    canonical = route46, bx = -15, by = -18 },
}

for _, pair in ipairs(legacyRoutes) do
  assert(MapAprons.canonicalMapId(pair.map) == pair.canonical.id,
    pair.map.id .. " did not canonicalize to " .. pair.canonical.id)
  assert(MapAprons.blockAt(pair.map, pair.bx, pair.by) == 0x05,
    pair.map.id .. " did not receive the shared forest")
  assert(MapAprons.cacheTag(pair.map) == MapAprons.cacheTag(pair.canonical),
    pair.map.id .. " and its newer alias produced different cache tags")
  local lx0, ly0, lx1, ly1 = MapAprons.tileBounds(pair.map)
  local cx0, cy0, cx1, cy1 = MapAprons.tileBounds(pair.canonical)
  assert(lx0 == cx0 and ly0 == cy0 and lx1 == cx1 and ly1 == cy1,
    pair.map.id .. " and its newer alias produced different mesh bounds")
end
assert(MapAprons.canonicalMapId("ROUTE45") == "ROUTE_45",
  "future compact numbered-route ids are not canonicalized")
assert(MapAprons.canonicalMapId("CHERRYGROVE_CITY") == "CHERRYGROVE_CITY",
  "canonicalization changed an already-stable city id")
...
for _, map in ipairs({ cherry, route29, route30, route31, route46 }) do
  assert(MapAprons.cacheTag(map):find("wall", 1, true),
    map.id .. " does not fingerprint the shared forest connector")
end
...
assert(mesherSource:find(
    "and not MapAprons.closesPerimeterAt(map, tx, ty)", 1, true),
  "real-neighbour height masks can erase the shared forest again")
assert(mesherSource:find(
    "local authoredVolume = MapAprons.closesPerimeterAt(map, tx, ty)",
    1, true)
    and mesherSource:find("and not authoredVolume", 1, true),
  "real-neighbour emission masks can erase the shared forest again")

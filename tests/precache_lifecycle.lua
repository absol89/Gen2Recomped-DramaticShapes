...
local bytes = {}
local function versionOf(scope)
  return scope and scope.save and scope.save.version or "unscoped"
end
local function bucket(scope)
  local version = versionOf(scope)
  bytes[version] = bytes[version] or {}
...
function storage:list(scope, prefix)
  local out = {}
  local under = prefix .. "/"
  for key in pairs(bucket(scope)) do
...
function storage:delete(_, key)
  local scoped = bucket(_)
  if scoped[key] == nil then return false, "not_found" end
  scoped[key] = nil
  return true
end
...
local function scoped(version)
  return { save = { version = version,
    meta = { playthroughId = "bavc_g2_static_mesh_v1" } } }
end
local map = {
  id = "TEST_MAP",
  def = {
...
local goldKeys = storage:list(scoped("gold"), Disk.DIRECTORY)
...
assert(#storage:list(scoped("silver"), Disk.DIRECTORY) == 0,
  "Silver saw records from the Gold namespace")
Disk.beginPrecache()
assert(Disk.saveTerrain(map, "full", {}, record(), { n = 0 }))
local silverKey = storage:list(scoped("silver"), Disk.DIRECTORY)[1]
assert(silverKey and bytes.silver[silverKey], "Silver record was not persisted")
...
local goldCleared = true
for _, key in ipairs(storage:list(scoped("gold"), Disk.DIRECTORY)) do
  if bytes.gold[key] ~= "" then goldCleared = false break end
end
assert(removed >= 3 and goldCleared,
  "DROP did not invalidate the Gold version cache")
assert(bytes.silver[silverKey] ~= nil, "Gold DROP removed the Silver cache")
